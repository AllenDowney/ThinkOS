...
Nevertheless, there are some syncrhonization problems that can be
...
to make it safer and easier to use.  My API is in {\tt sem.h}:
...
If {\tt sem\_init} returns 0 if it succeeds and -1 if anything
goes wrong.  One nice thing
about using wrapper functions is that you hide the error-checking
code from the user.
...
Note that you have to initialize the semaphore to 1 to indicate
that the mutex is initially unlocked; that is, one thread can
...
Using the semaphore implementation from the previous section, we can
...
{\tt mutex} is used to guarantee mutually-exclusive access to the queue;
the initial value is 1, again, so the mutex is
initially unlocked.
...
And here is the new version of \verb"queue_pop":
...
functions that implement the semaphore API in the previous section
using using condition variables and mutexes.  You can fill in your
...
becomes negative, the thread has to block until a ``wakeup'' is
...
and check whether a wakeup is still available.  If not, it
