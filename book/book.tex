...
Chapter 9 is about POSIX threads.

Chapter 10 is about synchronization with POSIX mutexes and
condition variables.
...
are in a Git repository at \url{https://github.com/AllenDowney/ThinkOS}.
...
If you think of the names as keys, and the the contents as values,
...
\item When the operating system handles the interrupt, is sees that
...
% TODO: is there a section about paging?
...
so threads can share dynamically-allocated
chunks.
...
When {\tt child\_code} returns, {\tt entry} invokes {\tt pthread\_exit}
which can be used to pass a value to the thread
that joins with this thread.  In this case, the child
...
If you have downloaded the repository for this book, you'll find
this example in {\tt counter/counter.c}.  You can compile and
run it like this:
...
If another thread executes \verb"mutex_lock" which another thread
is in the middle of \verb"child_code", it blocks.

When the first thread is done, it executes \verb"mutex_unlock",
which allows one other thread to proceed.  In effect, the threads
...
copy, which makes the mutex behave wrong.  To avoid that, you have to
pass \verb"pthread_mutex_t" by address.

My code makes it easier to get that right.  It defines a
type, {\tt Mutex}, which is a structure that contains a
\verb"pthread_mutex_t".  Actually, it contains an array of one
\verb"pthread_mutex_t", so when you allocate space for a
{\tt Mutex}, it allocates space for the ``array'' at the same
time.
...
\verb"make_mutex" creates a {\tt Mutex} struct and returns a
pointer to it.  You can pass this pointer as an argument without
...
The expression {\tt mutex->mutex} is not as strange as it seems:
{\tt mutex} is a pointer to {\tt Mutex}.  The field, {\tt mutex->mutex} is
a pointer to \verb"pthread_mutex_t", which is the type expected
by \verb"pthread_mutex_lock" and \verb"pthread_mutex_unlock".
...
A semaphore is a structure used to help threads work together
...
In my programs, I use capital letters to denote types 
that behave like structures, and I always manipulate them with pointers.
Fortunately, it is easy to put a wrapper around {\tt sem\_t}
to make it look like a proper structure.  Here is the 
...
about using wrapper functions is that you can wrap functions that
use different error reporting conventions and provide more
