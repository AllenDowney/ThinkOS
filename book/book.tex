...
This version of the queue is thread safe; if you run it, you
...
A condition variable does indicates whether a condition is
true and allows threads to block until the condition becomes true.
For example, \verb"thread_push" might want to check whether the
queue is full and, if so, block until it is not full.  So the
``condition'' we are interested in is ``queue not full''.
...
The Cond structure I used in the previous section is a is a wrapper
...
NOTE: This chapter is not done, and might get deleted.
