...
\item Donald Robertson spotted two typos.

\item Jim Tyson sent in two corrections.
...
are designed to fit neatly into disk blocks.  A UNIX inode
...
than column-wise, with a stride equal to the row length.

Linked data structures don't always exhibit spatial locality, because
the nodes aren't necessarily contiguous in memory.  But you allocate
...
is capable of {\bf multitasking}, which means it can switch from one
...
It's hard to specify which parts of the operating system should be
in the kernel.  But at its most basic, the kernel's job is to
...
\item When a process is interrupted and the schedule chooses not
...
As we saw in Section~\ref{unix_ps} there might be hundreds of
...
response is a matter of life and death, {\bf real-time operating
...
{\tt pthread\_t} is in all the implementations I know.}.  
You can think of a thread handle as an immutable
...
that it returns $-1$ if something went wrong.  One nice thing
